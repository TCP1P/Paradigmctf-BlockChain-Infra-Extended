...
async def deploy(clients: [Client], player_address: int) -> int:
    deployer = clients[0]
    bidder_1 = clients[1]
    bidder_2 = clients[2]

    print("[+] deploying erc20")
    erc20_deployment: DeployResult = await Contract.deploy_contract_v1(
        client=deployer,
        compiled_contract=Path("compiled/erc20.cairo").read_text(),
        constructor_args=[
            int_from_bytes(b"Test Token"),
            int_from_bytes(b"TTK"),
            6,
            int(1000000e6),
            deployer.address,
        ],
    ...
    auction_deployment: DeployResult = await Contract.deploy_contract_v1(
        client=deployer,
